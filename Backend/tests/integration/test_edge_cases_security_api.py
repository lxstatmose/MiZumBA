...
def test_reactions_are_idempotent_and_removable(client: TestClient) -> None:
    ...
    assert second.json()["action"] == "removed"
    assert first.json()["my_reaction"] == "👍"
    assert second.json()["my_reaction"] is None

    changed = client.post(f"/api/v1/messages/{message_id}/reactions", headers=bob_headers, json={"emoji": "❤️"})
    assert changed.status_code == 201
    assert changed.json()["action"] == "added"
    switched = client.post(f"/api/v1/messages/{message_id}/reactions", headers=bob_headers, json={"emoji": "👍"})
    assert switched.status_code == 201
    assert switched.json()["action"] == "changed"
    assert switched.json()["my_reaction"] == "👍"
    ...
    assert reactions.json()[0]["emoji"] == "👍"

    messages = client.get(f"/api/v1/chats/{chat.json()['id']}/messages", headers=alice_headers)
    assert messages.status_code == 200
    assert messages.json()[0]["my_reaction"] is None
    assert messages.json()[0]["reactions"][0]["emoji"] == "👍"
    assert messages.json()[0]["reactions"][0]["count"] == 1

    removed = client.request(
        "DELETE", f"/api/v1/messages/{message_id}/reactions", headers=bob_headers, json={"emoji": "👍"}
    )
