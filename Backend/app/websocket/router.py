...
from app.messages.realtime import broadcast_message_reactions_updated
from app.messages.service import create_message, set_message_reaction
...
async def _handle_message_reaction_set(session: Session, user: User, payload: dict) -> None:
    data = MessageReactionSetPayload.model_validate(payload)
    update = set_message_reaction(
        session,
        message_id=data.message_id,
        current_user=user,
        emoji=data.emoji,
    )
    await broadcast_message_reactions_updated(session, update=update, actor_user_id=user.id)
...
async def _handle_event(session: Session, user: User, websocket: WebSocket, raw_event: dict) -> None:
    ...
    elif event.type == "message.reaction.set":
        await _handle_message_reaction_set(session, user, event.payload)
